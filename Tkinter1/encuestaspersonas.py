...
class Product:
    ...
    def __init__(self, win2):
        ...
        tex=Label(self.wind, text="Registro De Familias",bg="#2e3192", fg="white", font=('Arial', 25, "bold" ))
        tex.grid(row = 0, column = 0, columnspan = 3, pady = 20)
        ...
        # ID Input
        Label(frame, text = 'Cédula: ').grid(row = 4, column = 0)
        self.cedula = Entry(frame)
        self.cedula.grid(row = 4, column = 1)

        # Sisben Input
        Label(frame, text = 'Tipo de sisbén: ').grid(row = 5, column = 0)
        self.sisben = Entry(frame)
        self.sisben.grid(row = 5, column = 1)

        # Edad Input
        Label(frame, text = 'Número de integrantes: ').grid(row = 6, column = 0)
        self.edad = Entry(frame)
        self.edad.grid(row = 6, column = 1)
        

        # Button Add Product 
        ttk.Button(frame, text = 'Guardar Encuesta', command = self.add_product).grid(row = 7, columnspan = 2)

        # Output Messages 
        self.message = Label(win2,text = '', fg = 'red')
        self.message.grid(row = 8, column = 0, columnspan = 5)

        # Table
        self.tree = ttk.Treeview(win2,height = 9,  columns = (1,2,3,4,5))
        ...
        self.tree.heading(4, text = 'Sisbén',anchor= CENTER)
        self.tree.heading(5, text = 'Integrantes',anchor= CENTER)
        
        # Buttons
        ttk.Button(win2,text = 'Borrar', command = self.delete_product).grid(row = 11, column = 0, sticky = W + E)
        ttk.Button(win2,text = 'Editar', command = self.edit_product).grid(row = 11, column = 1, sticky = W + E)
    ...
    def get_products(self):
        ...
        # getting data
        query = 'SELECT * FROM product ORDER BY sisben DESC,edad'
        db_rows = self.run_query(query)
        # filling data
        for row in db_rows:
            self.tree.insert('', 0,text=row[4],values=row[:5:1])
